...
$.omWidget("om.omDraggable", $.om.omMouse, {
	...
	options: {
		 ...
		 * @example //$("#selector")的子元素的&lt;p /&gt不能能够启动拖动操作
		 ...
		 * @type Selector
		 * @default 无
		 * @example //只有$("#selector")的子元素的&lt;p /&gt内才能够启动拖动操作
		 ...
		 * @default "original"
		 * @example //$("#selector")的clone元素将作为辅助元素
		 ...
		 * @event
		 * @param ui
		 *                Objec对象。包括四个属性：helper，position(当前位置)，originalPosition(原始位置)，offset(偏移量)
		 * @param event
		 *                jQuery.Event对象。
		 * @name omDraggable#onStart
		 * @type Function
		 * @example $("#selector").omDraggable({onStart : function(ui,
		 *          event) {doSomething...}});
		 */
		 ...
		 * @type Function
		 * @example $("#selector").omDraggable({onDrag : function(ui,
		 *          event) {doSomething...}});
		 */
		 ...
		 * @type Function
		 * @example $("#selector").omDraggable({onStop : function(ui,
		 *          event) {doSomething...}});
		 */
	},
	_create: function() {
	 ...
	 * @function
	 * @returns JQuery对象
	 * @example var $selector = $("#selector").omDraggable('destroy');
	 * 
	 */
	destroy: function() {
	...
	original:function(data){
		if(data){
			this.originalPosition.top = data.Y;
			this.originalPosition.left =data.X;
		}
	},
	...
	_trigger: function(type, event, ui) {
		...
		if(type == "onDrag") this.positionAbs = this._convertPositionTo("absolute"); // The
																						// absolute
																						// position
																						// has
																						// to
