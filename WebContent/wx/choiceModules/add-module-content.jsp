		...
		<div class="container-fluid">
			<div class="row-fluid div_title">
				...
				<div class="span1" style="padding-top:5px;">
					<button id="save_module_content_btn" type="button" class="btn btn-success pull-right">保存</button>
				</div>
			...
			<div class="padding_style">
				<div class="row-fluid div_header"> 
					<div class="span12">
						<label>内容模块名称</label>
						<input type="text" name="module_content_title" id="module_content_title" value="${wxModule.moduleName }">
					</div>				
				</div>
				<div id="add_content_container">
					
			</div>
			</div>
		</div>
	<script type="text/javascript">
		var path = "${ctx}/wx/modhtml/";
		...
		*/
		function renderPages() {
			...
			for (var i=0; i<pageCount; i++) {
				str += '<div class="row-fluid div_body">'+
							'<div class="span4" style="width:360px">'+
								'<div class="padding_style">'+
									'<iframe id="iframe_'+i+'" src="'+path + moduleGuid + '-' + i + '.jsp" width="330px" height="480px"></iframe>'+
								'</div>'+
							'</div>'+
							'<div class="span8" style="width:auto;min-width:500px">'+
								'<div class="padding_style" id="content_container_content_iframe_'+i+'">'+
			...
			for (var i=0; i<pageCount; i++) {
				$("#iframe_"+i).load(function(){
					...
					var dataInputs = $(me).contents().find("input[dataType=value]");
					for (var j=0; j<dataInputs.length; j++){
						...
						switch (selector.attr("inputType")) {
							...
							case "text":
								...
								break;
							case "textarea":
								dataStr += '<div><label>'+selector.attr("title")+' </label><textarea cols="30" rows="3"  belong="'+$(me).attr("id")+'"  id="content-'+i+'-'+j+'" name="'+targetId+'"></textarea></div>';
								break;
							case "pic":
								dataStr += '<div><label>'+selector.attr("title")+' </label><img alt="" src="" width="100" height="100"/><input id="fileExtend-'+i+'-'+j+'"/><input class="uploadImg" type="hidden" belong="'+$(me).attr("id")+'" name="'+targetId+'"></div>';
								break;
							case "date":
								dataStr += '<div><label>'+selector.attr("title")+' </label><input id="dateTime-'+i+'-'+j+'" type="text" belong="'+$(me).attr("id")+'" name="'+targetId+'"></div>';
								break;
							case "flash":
								dataStr += '<div><label>'+selector.attr("title")+' </label><input type="text" belong="'+$(me).attr("id")+'" name="'+targetId+'"></div>';
								break;	
					...
					$("#content_container_content_iframe_"+index).html(dataStr);
		...
		//
		$("#save_module_content_btn").click(function(){
			var datas = [];
			var urls = [];
			for (var i=0; i<pageCount; i++) {
				var data = $("#iframe_"+i).contents().find("#data").text();
				var dataInputs = $("#iframe_"+i).contents().find("input[dataType=value]");
				for (var j=0; j<dataInputs.length; j++){
					var selector = $(dataInputs[j]);
			...
			$.ajax({
				  type: 'POST',
				  url: "${ctx}/ajax/saveModuleContent",
				  data: {result:JSON.stringify(datas),urlResult:JSON.stringify(urls),moduleGuid:moduleGuid,contentTitle:title,pageCount:pageCount},
				  dataType: "json",
