  ...
  <body>	
    ...
    ================================================== -->
     ...
     <script type="text/javascript">
		...
		for (i=0; i<length; i++){
			if (i==0) {
				elementStr = '<a href="#" id="item_'+i+'">'+
								'<div class="div_header align_horizontal_center">'+
									'<div class="header_pic">'+
										'<img alt="" src="' + defaultPicSrc + '" height="200" width="320">'+
										'<div class="img_caption">'+
											'<span>' + defaultTitle + i + '</span>'+
										'</div>'+
									'</div>'+
								'</div>'+
							'</a>'+
							'<input type="hidden" targetId="item_' + i + '" urlType="url" title="'+ defaultTitle + i + " 跳转至" +'"/>';
			} else {
				elementStr = '<a href="#" id="item_'+i+'">'+
								'<div class="'+(i==length-1? "div_footer": "div_body")+'" >'+
									'<div>'+
										'<span>' + defaultTitle + i + '</span>'+
									'</div>'+
									'<div class="float_right_img" width="70xp" height="70xp">'+
										'<img alt="" src="' + defaultPicSrc + '" width="70xp" height="70xp">'+
									'</div>'+
								'</div>'+
							'</a>'+
							'<input type="hidden" targetId="item_'+i+'" urlType="url" title="'+ defaultTitle + i + " 跳转至" +'"/>';
		...
		// 此处代码 显示如何添加链接
		// hiddenInputs = $("input[urlType=url]");
		// console.log($(hiddenInputs[0]));
		// var str2 = "";
		// for (i=0; i<hiddenInputs.length; i++){
			// var selector = $(hiddenInputs[i]);
			// elementStr = "<div><label>"+selector.attr("title")+"</label> <input type='text' name='"+selector.attr("targetId")+"' /></div>";
			
			// str2 += elementStr;
		// }
		// $("#define_url").html(str2);
    </script>
