...
<div class="row-fluid div_title">
	...
	<script type="text/javascript">
		...
		if (window.localStorage && window.localStorage.result) {
			...
			console.log("get data:", result.content);
		}
		var params = [];
		var keyName;
		data.push(result);
		...
		$.ajax({
			type : "POST",
			url : "${ctx}/ajax/obtainNews",
			data : {
				newsGuid : result.newsGuid
			},
			async : false,
			success : function(msgData) {
				...
				if (msgData.success) {
					for ( var i = 0; i < msgData.result.length; i++) {
						data.push(msgData.result[i]);
					}
					for (var i=0; i<data.length; i++) {
						if (data[i].url.indexOf("http")>-1) {
							data[i].outlink = data[i].url;
							data[i].url = "outlink";
						} else {
							data[i].outlink = "";
						}
					...
					editor = KindEditor.create("#content", {
						...
						items : [ 'undo', 'redo', '|', 'formatblock', 'fontname',
								...
								'outdent', '|', 'image','h5player', 'table', 'hr', 'emoticons',
								'link', 'unlink', '|', 'preview', 'plainpaste', '|',
								'removeformat', 'fullscreen', 'module' ],
					...
					$.ajax({
						 ...
						 success:function(res){
							...
							for(var i=0;i<msg.length;i++){
								if(msg[i].id){
									if (msg[i].keyName==data[index].urlParamName) {
		...
		function assignContent() {
			...
			for (var i = 0; i < data.length; i++) {
				if (i == 0) {
					elementStr = '<a href="#" id="item_'
							+ i
							+ '" onclick="itemClick('
							+ i
							+ ')">'
							+ '<div class="header_pic_outer align_horizontal_center">'
							+ '<div class="header_pic">'
							+ '<img alt="" id="pic_'
							+ i
							+ '" src="${ctx}'
							+ (data[i].picSrc ? data[i].picSrc : defaultPicSrc)
			...
			$("input[name=messageTitle]").val(data[0].name);
		}
		...
		function showRightContent() {
			...
			$("#preview").attr(
					"src",
					data[index].picSrc ? ("${ctx}" + data[index].picSrc)
							: "${ctx}" + defaultPicSrc);
			//$("#content").val(data[index].content);
			console.log("content:",data[index].content, index, data);
		...
		$("#save_page_content_btn").click(function() {
			...
			$.ajax({
				type : "POST",
				url : "${ctx}/ajax/editNews",
				data : {
					title : $("input[name=messageTitle]").val(),
					data : JSON.stringify(data),
					deldata : JSON.stringify(dels)
				},
		...
		$("#remove_page_content_btn").click(function() {
			if (data.length > 1) {
				var guid = data[data.length - 1].newsGuid;
				if (guid) {
					dels.push({"newsGuid":guid});
				}
				data.pop();
				assignContent();
			} else {
		...
		$.ajax({
			...
			success : function(msg) {
				...
				console.log("ajax:",data[index]);
				if (data[index].url.indexOf("http")>-1 || data[index].url=="outlink") {
					html +='<option value="outlink" selected>外部链接</option>';	
					$("#outlink").val(data[index].outlink);
					$("#outlink").show();
				} else {
