...
<body>
	...
	<div id="search-panel">
		<form id="searchForm" name="searchForm">
			...
			<div>
				...
				<div class='input-label'><span class='label'>用户类型</span><span><input type="text" name="memberLevel" id="memberLevel" class="input-text" /></span></div>
			</div>
			<div>
			...
			<div>
				<div class='input-label'><span class='label'>注册时间</span><span><input type="text" name="zcStartDate" id="zcStartDate" class="input-text" /> 至 <input type="text" name="zcEndDate" id="zcEndDate" class="input-text" /></span></div>
			</div>
			<div>
	...
	$(document).ready(function() {
	    ...
	    $("#zcStartDate").omCalendar({
               dateFormat : "yy-mm-dd H:i:s",
               showTime : true
           });
	    $("#zcEndDate").omCalendar({
               dateFormat : "yy-mm-dd H:i:s",
               showTime : true
           });
	    ...
	    $('span#button-search').omButton({
	        ...
	        onClick: function(event){
				...
				$('#grid').omGrid('setData', '${ctx}/ajax/searchdata?param=reg&data='+encodeURIComponent(JSON.stringify(data)));
	        }
	    });
	    ...
	    $('#grid').omGrid({
	        ...
	        autoFit: true,
	        showIndex: false,
	        dataSource: "${ctx}/ajax/griddata.action?type=wxuser&param=reg",
	        colModel: [{
	            header: '用户名',
	            name: 'userName',
	            align: 'left',
	        ...
	        },
	        {
	            ...
	            width: 100,
	            renderer: function(value, rowData, rowIndex) {
	                if (rowData.carNumberPfx) {
	                    return  rowData.carNumberPfx + value;
	                }
	                return value;
	            }
	        },
	    ...
	    $('#buttonbar').omButtonbar({
	        btns: [{
	            ...
	            onClick: function() {
    	        	...
    	        	var form = new Util.Form("searchForm");
    	        	var data = form.getValues();
    	        	location.href = '${ctx}/export/export?param=reg&data='+encodeURIComponent(JSON.stringify(data));
	            }
	        },{
	            label: "删除",
	            id: "button-remove",
