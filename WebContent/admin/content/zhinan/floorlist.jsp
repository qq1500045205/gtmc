...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#grid').omGrid({
			...
			height:400,
			showIndex : false,
			dataSource : "${ctx}/ajax/griddata.action?type=contentFloor",
			colModel : [ {
				header : '楼层名称',
				name : 'floorName',
				align : 'left',
				...
				sort : 'clientSide',
				renderer:function(colValue, rowData, rowIndex){
               		return '<a href="${ctx}/'+rowData.floorPic+'" target="_blank">查看</a>';
         		}  
			}, {
				header : '创建人',
				name : 'createdBy',
 	                ...
 	                onClose:function(v){
 	                   if(v){
 	                	   ...
 	                	   for(var i=0;i<dels.length;i++){
 	                		   items += "items="+dels[i].floorGuid;
 	                		   if(i< dels.length-1){
 	                			   items +="&";
 	                		   }
 	                	   }
 	                	  $.getJSON("${ctx}/ajax/deleteFloor?"+items,function(data){
 	                		  if(data.success){
 	                			 $.omMessageBox.alert({
         	                         type:'success',
         	                         title:'成功',
...
<body>
	...
	<div id="dialog" title="添加楼层" style="overflow:hidden">
       <form id="addFloorForm" action="${ctx}/ajax/addFloor" method="post">
		    <div id="nav_cont">
		        <table class="grid_layout">
		            <tr>
		                <td width="80px"><span class="color_red">*</span>楼层名称：</td>
		                <td ><input name="floorName" class="input-text"/></td>
		                <td width="30px"><span class="errorImg"></span><span class="errorMsg" ></span></td>
		            </tr>
		            <tr>
		                <td width="80px"><span class="color_red">*</span>楼层描述：</td>
		                <td ><textarea name="floorDesc" cols="30" rows="3"></textarea></td>
		                <td width="30px"><span class="errorImg"></span><span class="errorMsg" ></span></td>
		            </tr>
		            <tr>
		                ...
		                <td>
		                	...
		                	<input id="fileExtend"/>
		                	<input name="floorPic" type="hidden" id='image'/>
