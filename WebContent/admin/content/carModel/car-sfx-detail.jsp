  ...
  <head>
   ...
   <body>	
			...
			<div class="span6">
				<div class="accordion">
					<div class="accordion-group padding_style" id="content_container_edit">
						<div><label>名称</label><input type="text" name="name" id="name" ></div>
						<!--<div><label>颜色</label><input type="text" name="color" id="color" style="width: 100%;"></div>  -->
						<div><label>价格</label><input type="text" name="price" id="price" style="width: 100%;"></div>
						<div><label>显示排序(0表示最前面)</label><input type="text" name="inputSortId" id="inputSortId"  style="width: 100%;" title="0表示最前面" value="0">
						</div>
					</div>
				</div>
			</div>
    ...
    $(function() {
		...
		$.ajax({
			 type:"POST",
			 url:"${ctx}/car/queryCarSfx",
			 data:{carSfx:JSON.stringify(data)},
		...
		function setData(data){
			$("#name").val(data.carName);
			//$("#color").val(data.carColor);
			$("#price").val(data.carPrice);
			$("#inputSortId").val(data.carSortid);
		}
	});
