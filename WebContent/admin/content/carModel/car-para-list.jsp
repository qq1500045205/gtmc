...
<script type="text/javascript">
	$(document).ready(function() {
		$('#grid').omGrid({
			...
			height:400,
			showIndex : false,
			dataSource : "${ctx}/car/getCarParaListByNavi?carParaGuid1=${param.carParaGuid}",
			colModel : [ {
				header : '参数名',
				name : 'paraName',
				align : 'left',
		...
		$('#buttonbar').omButtonbar({
			btns : [ {
				...
				onClick : function() {
					...
					$("#dialogIframe").attr("src", "${ctx}/admin/content/carModel/add-car-para.jsp?carParaGuid=${param.carParaGuid}&carGuid=${param.carGuid}");
				}
			},
 			{separtor:true},
 	        {label:"删除",
 	 		 ...
 	 		 onClick:function(){
 	         	...
 	         	if(dels.length <= 0 ){
 	         	 ...
 	         	 $.omMessageBox.confirm({
 	                title:'确认删除',
 	                content:'您确定要删除？<p style="color:red">注意：删除参数名后,不可恢复,请谨慎操作。<p>',
 	                onClose:function(v){
 	                   if(v){
 	                	   var items="";
 	                	  ...
 	                	  $.getJSON("${ctx}/car/deleteCarPara?"+items,function(data){
 	                		  if(data.success){
 	                			 $.omMessageBox.alert({
         	                         type:'success',
         	                         title:'成功',
    ...
    function showResponse(responseText, statusText, xhr, $form) {
	 ...
	 function CheckTabsExist(currentTabId)
     {
  	...
  	function viewCarPara(carParaGuid){
  		...
  		$("#dialogViewIframe").attr("src", "${ctx}/admin/content/carModel/car-para-detail.jsp?carParaGuid='"+carParaGuid+"'");
  	}
