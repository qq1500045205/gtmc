...
<script type="text/javascript">
	$(document).ready(function() {
		$('#grid').omGrid({
			...
			height:400,
			showIndex : false,
			dataSource : "${ctx}/car/getCarSfxListByModel?carTypeGuid=${param.carTypeGuid}",
			colModel : [ {
				header : '型号名称',
				name : 'carName',
				align : 'left',
				...
				renderer:function(colValue, rowData, rowIndex){//dataSource的action对应的结果
					return '<a href="#" onclick=addCarNavi("'+rowData.carGuid+'")>'+rowData.carName+'</a>';
         		} 
			},{
				header : '型号价格(万)',
				name : 'carPrice',
				...
				sort : 'clientSide',
				renderer:function(colValue, rowData, rowIndex){
					return '<a href="#" onclick=viewCarSfx("'+rowData.carGuid+'")>查看</a>';
         		}  
			}],
			autoFit : true
		});
		...
		$('#buttonbar').omButtonbar({
			btns : [ {
				...
				onClick : function() {
					...
					$("#dialogIframe").attr("src", "${ctx}/admin/content/carModel/add-car-sfx.jsp?carTypeGuid=${param.carTypeGuid}");
				}
			},
 			{separtor:true},
 	        {label:"删除",
 	 		 ...
 	 		 onClick:function(){
 	         	...
 	         	if(dels.length <= 0 ){
 	         	 ...
 	         	 $.omMessageBox.confirm({
 	                title:'确认删除',
 	                content:'您确定要删除？<p style="color:red">注意：删除车型信息后,对应的导航参数类型信息和参数名也将被删除,不可恢复,请谨慎操作。<p>',
 	                onClose:function(v){
 	                   if(v){
 	                	   var items="";
 	                	   for(var i=0;i<dels.length;i++){
 	                		   items += "items="+dels[i].carGuid;
 	                		   if(i< dels.length-1){
 	                			   items +="&";
 	                		   }
 	                	   }
 	                	  $.getJSON("${ctx}/car/deleteCarSfx?"+items,function(data){
 	                		  if(data.success){
 	                			 $.omMessageBox.alert({
         	                         type:'success',
         	                         title:'成功',
    ...
    function showResponse(responseText, statusText, xhr, $form) {
	 ...
	 function CheckTabsExist(currentTabId)
     {
  	...
  	function addCarNavi(carGuid){
  		var tabId = "mgr-car-navi-tab-id";
  		if (CheckTabsExist(tabId))
        {
       	...
       	parent.$("#make-tab").omTabs('add',{
	           title : "型号导航属性管理", 
	           content : "<iframe id='inner-frame' border=0 frameBorder='no'  name='inner-frame' src='${ctx}/admin/content/carModel/car-navi-list.jsp?carGuid="+carGuid+"' width='100%' height='600'></iframe>",
	           closable : true,
	           tabId:tabId
	     	});
  	...
  	function viewCarSfx(carGuid){
  		$("#dialogview").omDialog('open');
  		$("#dialogViewIframe").attr("src", "${ctx}/admin/content/carModel/car-sfx-detail.jsp?carGuid='"+carGuid+"'");
  	}
...
<body>
	...
	<div id="dialog" title="添加型号信息" style="overflow:hidden">
	<iframe id=dialogIframe src="${ctx}/admin/content/carModel/add-car-sfx.jsp" height="100%" width="100%">
	</iframe>
    </div>
    
    <div id="dialogview" title="查看型号信息" style="overflow:hidden">
