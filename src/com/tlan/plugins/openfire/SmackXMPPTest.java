...
public class SmackXMPPTest {
	...
	/** openfire服务器address */
	private final static String server = "115.28.20.72";

	// private int SERVER_PORT = 5222;
	// private String SERVER_HOST = "115.28.20.72";
	// private String SERVER_NAME = "ay131227154706800b19z";
	...
	public void testAccountManager() {
		...
		fail("supportsAccountCreation: {0}",
	...
	public void testPacket() {
		...
		Message message = new Message("boy@" + server, Type.chat);
	...
	public void testChatManager() {
		...
		Chat chat = chatManager.createChat("boy@" + server+"/spark",
				new MyMessageListeners());
		try {
			/** 发送消息 */
			chat.sendMessage("h!~ guojing……");
...
//			chat.sendMessage(message);
	...
	class MyMessageListeners implements MessageListener {
		public void processMessage(Chat chat, Message message) {
			...
			try {
				/** 发送消息 */
				System.out.println("dingding……" + message.getBody());
				chat.sendMessage("dingding……ddddd"); //+ message.getBody());
			} catch (XMPPException e) {
				e.printStackTrace();
			}
			/** 接收消息 */
