...
public class WeixinAction extends BaseAction implements Preparable,
		...
		try {
			...
			if (msg instanceof WxRecvEventMsg) {
				...
				case TypeBean.SUBSCRIBE:
					...
					//add by lyn 二维码
					log.info("##################二维码扫描####################");
					getActives(sendMsg,m.getEventKey().substring(8));
					//sendMsg = getActives(sendMsg,m.getEventKey());
...
//					sendMsg = obtainContent(sendMsg, m.getEventKey());
				...
				case TypeBean.SCAN:
					//add by lyn 二维码
					log.info("##################二维码扫描####################" + m.toString());
					log.info("--------23423432423----------------eventKey:" + m.getEventKey());
					//sendMsg = getActives(sendMsg,m.getEventKey());
					//WeiXin.send(sendMsg, response.getOutputStream());
					getActives(sendMsg,m.getEventKey());
					break;
				// 上报地理位置
	...
	private WxSendMsg dealTextMsg(WxSendMsg sendMsg, WxRecvMsg msg) {
		...
		String suggest_url = "http://" + PropertiesUtil.getProperty(ParamString.BASE_IP)+ "/gtmc_wx/"
	            +ModuleAction.ADMIN_HTML+"suggest-page.jsp";
		//print("");
		List<WxUserStatus> wxUserStatuses = wxUserStatusService.getAll(
				WxUserStatus.class, "openId", openid);
...
//		log.info("进入人工客服的关键字CRKeyWord:" + CRKeyWord);
	 ...
	 public WxSendMsg setInfo(WxSendMsg sendMsg) {
		 	String url = "http://" + PropertiesUtil.getProperty(ParamString.BASE_IP)+ "/gtmc_wx/"
		            +ModuleAction.ADMIN_HTML+"liuzi-page.jsp";
...
//	    }
		 ...
		 * @return
		 */
		private String getActives(WxSendMsg sendMsg, String eventKey) {
				log.info("--------扫描二维码----------------eventKey:" + eventKey);
				String openid = sendMsg.getToUser();
				log.info("--------扫描二维码----------------openid:" + openid);
				String gzhHash = weixinBean.getHash();
				log.info("--------扫描二维码----------------gzhHash:" + gzhHash);
				String imageUrl = "";
				String activeUrl = "";
		    	String sendAPI = PropertiesUtil.getProperty("send_customer_message_ur");
				WxGongzhonghao m = gzhService.get(WxGongzhonghao.class, "gzh_hash", gzhHash);
				String accessToken = HttpUtils.getToken(m.getAppid(), m.getAppsecret());
		    	try{
					List<WxMemberCreditActiveF> actives = WxMemberCreditActiveFService.getAll(
							WxMemberCreditActiveF.class,new String[] {"gzhHash","sceneId"}, new Object[] {gzhHash,Integer.parseInt(eventKey)});
					log.info("--------actives.size()---------------:" + actives.size());
					if (actives.size() > 0) {
						WxMemberCreditActiveF active = actives.get(0);
						log.info("--------扫描active维码----------------:" + active.getActImage());
						log.info("--------扫描active二维码----------------:" + active.getActDesc());
					    imageUrl = "http://" + PropertiesUtil.getProperty(ParamString.BASE_IP)+ "/gtmc_wx"
			                    + active.getActImage();
					    log.info("--------imageUrl----------------imageUrl:" + imageUrl);
					    activeUrl = "http://" + PropertiesUtil.getProperty(ParamString.BASE_IP)+ "/gtmc_wx/wx/html/integralActivities.jsp"
					    		+"?openid="+ openid+"&guid="+ active.getMemberCreditGuid()+"&hash="+ gzhHash;
					    log.info("--------activeUrl----------------activeUrl:" + activeUrl);
				    	sendAPI =  MessageFormat.format(sendAPI, accessToken);
//				    	String msgJson = "{\"touser\":\"%s\",\"msgtype\":\"text\",\"text\":{\"content\":\"%s\"}}";
				    	String msgJson = "{\"touser\":\"%s\",\"msgtype\":\"news\",\"news\":{\"articles\":[{\"title\":\"%s\",\"description\":\"%s\",\"url\":\"%s\",\"picurl\":\"%s\"}]}}";
				    	msgJson = String.format(msgJson, openid,active.getActName(),active.getActDesc(), activeUrl, imageUrl);
				    	HttpClientUtil.sendPostRequestByJava(sendAPI,msgJson);
					}


		    	}catch(Exception e){
		    		print("打印发送失败log开始------扫描二维码-------------");
		    		e.printStackTrace();
		    		print("-1");
		    	}
